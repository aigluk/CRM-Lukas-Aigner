...
export async function GET() {
  try {
    ...
    console.error('[GET /api/accounting/partners]', err?.message)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await db().from('accounting_partners').insert(row).select().single()
    if (error) {
      if (error.code === '42P01') return NextResponse.json({ error: 'SETUP_REQUIRED' }, { status: 503 })
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    ...
    console.error('[POST /api/accounting/partners]', err?.message)
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    console.error('[PATCH /api/accounting/partners]', err?.message)
    return NextResponse.json({ error: err?.message ?? 'Interner Serverfehler' }, { status: 500 })
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { error } = await db().from('accounting_partners').delete().eq('id', id).eq('user_id', ownerId)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })

    return NextResponse.json({ success: true })
  } catch (err: any) {
    console.error('[DELETE /api/accounting/partners]', err?.message)
    return NextResponse.json({ error: err?.message ?? 'Interner Serverfehler' }, { status: 500 })
  }
}
