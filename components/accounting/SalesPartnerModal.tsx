...
export function SalesPartnerModal({
  salesPartner, onClose, onSaved,
}: {
  salesPartner?: AccountingSalesPartner
  onClose: () => void
  onSaved: () => void
}) {
  const isEdit = !!salesPartner
  const [name, setName] = useState(salesPartner?.name ?? '')
  const [contactPerson, setContactPerson] = useState(salesPartner?.contact_person ?? '')
  const [address, setAddress] = useState(salesPartner?.address ?? '')
  ...
  const [notes, setNotes] = useState(salesPartner?.notes ?? '')
  ...
  function saveReminder() {
    if (!reminderText.trim() || !salesPartner?.id) return
    addReminder({
      refType: 'sales_partner',
      refId: salesPartner.id,
      refName: salesPartner.name,
  ...
  async function handleDelete() {
    if (!salesPartner?.id || deleting) return
    if (!confirm(`"${salesPartner.name}" wirklich löschen?`)) return
    setDeleting(true)
    try {
      const res = await fetch(`/api/accounting/sales-partners?id=${salesPartner.id}`, { method: 'DELETE' })
  ...
  async function save() {
    ...
    try {
      const res = await fetch('/api/accounting/sales-partners', {
        method: isEdit ? 'PATCH' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(isEdit ? { id: salesPartner!.id, ...payload } : payload),
      })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(data.error || 'Speichern fehlgeschlagen.')

      const originalNotes = salesPartner?.notes ?? ''
      if (notes.trim() && notes.trim() !== originalNotes.trim()) {
        const id = salesPartner?.id ?? data.salesPartner?.id
        if (id) addReminder({ refType: 'sales_partner', refId: id, refName: name.trim(), text: notes.trim() })
      }
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-panel z-10 px-5 pt-4 pb-3 border-b border-rim-subtle flex items-center justify-between gap-3">
          <h2 className="text-base font-black text-white flex-1 truncate">{isEdit ? 'Vertriebspartner bearbeiten' : 'Neuer Vertriebspartner'}</h2>
          {isEdit && (
            <button
          ...
          {isEdit && (
            <button
              ...
              disabled={deleting}
              title="Vertriebspartner löschen"
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>Name / Person</label>
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Max Mustermann" className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Ansprechperson</label>
              <input type="text" value={contactPerson} onChange={e => setContactPerson(e.target.value)} placeholder="falls abweichend" className={inputCls} />
            </div>
          </div>
          <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={website} onChange={e => setWebsite(e.target.value)} placeholder="www.beispiel.at" className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="kontakt@beispiel.at" className={inputCls} />
            </div>
            <div>
          ...
          >
            <Save size={14} />
            {saving ? 'Speichern…' : 'Vertriebspartner speichern'}
          </button>
