...
export function TodayPanel({ leads: initialLeads, onEdit }: Props) {
  ...
  return (
    <>
      ...
      <div
        className="bg-[#C7C7C7] rounded-2xl p-5 flex flex-col h-full"
      ...
      >
        ...
        <div className="flex items-center gap-4 mb-4 shrink-0">
          ...
          <div className="min-w-0">
            <p className="text-base font-black text-dark capitalize leading-tight">{weekday}</p>
            <p className="text-xs text-dark/50 capitalize mt-0.5">{monthYear}</p>
            {appts.length > 0 && (
        ...
        <div className="flex-1 overflow-y-auto min-h-0 space-y-2">
          {appts.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-2 py-4">
              <div className="w-14 h-14 rounded-2xl bg-dark/8 flex items-center justify-center">
                <CalendarDays size={26} strokeWidth={2.2} className="text-dark/40" />
              </div>
              <p className="text-xs text-dark/40 font-medium">Keine Termine heute</p>
            </div>
          ) : (
            appts.map(l => (
              <div
                ...
                className="flex items-center gap-3 bg-white rounded-xl px-3 py-2.5 hover:bg-white/70 transition-colors cursor-pointer group"
              >
                <div className="w-3 h-3 rounded-full bg-accent shrink-0" />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-dark truncate">{l.name}</p>
                  {l.phone && (
                    <a
                      href={`tel:${l.phone}`}
                      onClick={e => e.stopPropagation()}
                      className="flex items-center gap-1 text-xs text-dark/45 hover:text-accent transition-colors"
                    >
                      <Phone size={11} strokeWidth={2.5} />{l.phone}
                    </a>
                  )}
                </div>
                <Pencil size={13} strokeWidth={2.4} className="text-dark/25 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
              </div>
            ))
          )}
        </div>
