...
export function TimePicker({ value, onChange, label }: { value: string; onChange: (v: string) => void; label?: string }) {
  ...
  return (
    <div className="relative" ref={wrapRef}>
      {label && <label className="block text-[10px] font-black text-white/25 uppercase tracking-widest mb-1.5">{label}</label>}
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
  ...
  return (
    <div>
      <div className="grid grid-cols-7 mb-1">
        {WEEKDAYS.map(d => (
          <span key={d} className="text-[9px] font-black text-white/20 text-center uppercase">{d}</span>
        ))}
      </div>
